...
$uploads_dir = 'ir/input';
$error = "";
$looping = false;

if (isset($_GET["filename"]) && file_exists($uploads_dir."/".$_GET["filename"])) {
	$newfilename = $_GET["filename"];
	$looping = true;
}
else {
	$newfilename = str_replace(".wav", "", $_FILES["soundinput"]["name"])."-".time().".wav";
	$newfilename = preg_replace("/[^A-Za-z0-9._-]/","_",$newfilename);

	if ($_FILES["soundinput"]["type"] != "audio/wav") {
		$error = "You must upload a sound in WAV format.";
	}
	else {
		if (move_uploaded_file($_FILES["soundinput"]["tmp_name"], $uploads_dir."/".$newfilename)) {
			// all good!
			$AF = new AudioFile;
			$AF->loadFile($uploads_dir."/".$newfilename);
			echo $newfilename." is a ".$AF->wave_bits."/".$AF->wave_framerate."khz WAV file. Good!<br /><br />";
		
	   } else {
		   $error = "Failed to move file.<br />";
		   // all bad!
...
<div id="pagecontainer">
    ...
    <h2>CON-VOLV-O</h2></div>
		...
		<div id="instructions" class="poemaudio-center">
            ...
            <p>YOU UPLOADED: <strong>"<?php echo ($looping == true)?$newfilename:$_FILES["soundinput"]["name"]; ?>"</strong> <?php echo ($looping == true)?"":"(".$_FILES["soundinput"]["size"]."K)"; ?></p> <p><button class="btn" onclick="listen();">LISTEN</button>&nbsp;&nbsp;&nbsp;<button class="btn" onclick="if (confirm('Are you sure?')) { document.location.href='remove.php?filename=<?php echo $newfilename; ?>'; }">UPLOAD A DIFFERENT FILE</button></p>
            ...
            <input type="hidden" name="filename" id="filename" value="<?php echo $newfilename; ?>" ? />
            <select name="reverb" id="reverb">
            <option value="bachman" selected="selected">Bachman Hall</option>
            <option value="bilger">Bilger Hall</option>
            <option value="hamilton">Hamilton Library</option>
            <option value="kuystairwell">Kuykendall stairwell</option>
            <option value="moore">Moore Hall</option>
            </select>
            </p>
            <p><button class="btn" onclick="document.forms[0].submit();">CONVOLVE IT!</button></p>
            </form>
            <?php else: ?>
            
            <p><?php echo $error; ?></p>
            <p><button class="btn" onclick="document.location.href='convolvo.php';">TRY AGAIN</button></p>
